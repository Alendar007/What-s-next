...
export default async function handler(req, res) {
  ...
  try {
    // 1. Actualités
    if (news === 'true' && title) {
      const queryTerm = `${title} ${isManga ? 'manga webtoon' : 'anime'}`;
    ...
    // 2. Similaires (perPage augmenté à 50 pour capturer plus de séries récentes)
    if (similar === 'true' && id) {
      const graphqlQuery = `
        query ($id: Int) {
          Media(id: $id) {
    ...
    // 3. Recherche standard (Recherche sans restriction de type strict si Manga pour inclure Webtoons/Manhwas)
    if (search) {
      const graphqlQuery = `
