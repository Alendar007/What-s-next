...
export default async function handler(req, res) {
  ...
  try {
    // 1. Recherche d'une série (Autocomplete / Sélection)
    if (search) {
      const url = `${TMDB_BASE}/search/tv?api_key=${TMDB_API_KEY}&language=fr-FR&query=${encodeURIComponent(search)}&page=1`;
      const response = await fetch(url);
      if (!response.ok) throw new Error(`TMDB HTTP ${response.status}`);
      const data = await response.json();

      const results = await Promise.all((data.results || []).slice(0, 6).map(async item => {
        const detailsRes = await fetch(`${TMDB_BASE}/tv/${item.id}?api_key=${TMDB_API_KEY}&language=fr-FR&append_to_response=watch/providers`);
    ...
    // 2. Séries similaires / recommandées avec leurs plateformes en France
    if (similar === 'true' && id) {
      let recUrl = `${TMDB_BASE}/tv/${id}/recommendations?api_key=${TMDB_API_KEY}&language=fr-FR&page=1`;
      let response = await fetch(recUrl);
      let data = await response.json();
      let rawResults = data.results || [];

      if (rawResults.length === 0) {
        recUrl = `${TMDB_BASE}/tv/${id}/similar?api_key=${TMDB_API_KEY}&language=fr-FR&page=1`;
        response = await fetch(recUrl);
        data = await response.json();
        rawResults = data.results || [];
      }

      const results = await Promise.all(rawResults.filter(item => item.poster_path).slice(0, 30).map(async item => {
        const detailsRes = await fetch(`${TMDB_BASE}/tv/${item.id}?api_key=${TMDB_API_KEY}&language=fr-FR&append_to_response=watch/providers`);
        if (!detailsRes.ok) return item;
        const details = await detailsRes.json();
        const frProviders = details['watch/providers']?.results?.FR?.flatrate || [];
